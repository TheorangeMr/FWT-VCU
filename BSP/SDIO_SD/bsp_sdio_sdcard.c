...
********************************************************************************
    ...
    *文件名：bsp_sdio_sdcard.c
    *版  本：v1.0
    *日  期：2020.12.09
    *作  者：王峰
    *说  明：本文件是对ST固件库中的stm32_eval_sdio_sd.c和stm3210e_eval.c的整合
...
SD_Error SD_Init(void)
{
  ...
  /* 配置SDIO相应引脚工作模式及相关时钟使能 */
  SD_LowLevel_Init();
  
  /* 复位SDIO寄存器 */
  SDIO_DeInit();

  errorstatus = SD_PowerON(); //上电识别
  ...
  SDIO_InitStructure.SDIO_ClockDiv = SDIO_TRANSFER_CLK_DIV;                       // SDIO数据传输速度24Mhz                          
  SDIO_InitStructure.SDIO_ClockEdge = SDIO_ClockEdge_Rising;                      //上升沿采样
  SDIO_InitStructure.SDIO_ClockBypass = SDIO_ClockBypass_Disable;                 //不使能旁路
  SDIO_InitStructure.SDIO_ClockPowerSave = SDIO_ClockPowerSave_Disable;           //不使能节能模式
  SDIO_InitStructure.SDIO_BusWide = SDIO_BusWide_1b;                              //数据线宽度1位
  ...
  if (errorstatus == SD_OK)
  {
    errorstatus = SD_EnableWideBusOperation(SDIO_BusWide_4b); //使能SDIO数据线宽度为4位
  }
...
SD_Error SD_PowerON(void)
{
  ...
  SDIO_InitStructure.SDIO_ClockDiv = SDIO_INIT_CLK_DIV;                           //卡识别模式速度400khz
  SDIO_InitStructure.SDIO_ClockEdge = SDIO_ClockEdge_Rising;                      //上升沿采样
  SDIO_InitStructure.SDIO_ClockBypass = SDIO_ClockBypass_Disable;                 //不使能旁路
  SDIO_InitStructure.SDIO_ClockPowerSave = SDIO_ClockPowerSave_Disable;           //不使能节能模式
  SDIO_InitStructure.SDIO_BusWide = SDIO_BusWide_1b;                              //数据线宽度为1位
  SDIO_InitStructure.SDIO_HardwareFlowControl = SDIO_HardwareFlowControl_Disable; //不使能SDIO硬件流控制
  SDIO_Init(&SDIO_InitStructure);
  ...
  SDIO_CmdInitStructure.SDIO_CmdIndex = SD_CMD_GO_IDLE_STATE;    //复位所有的卡到 idle 状态
  SDIO_CmdInitStructure.SDIO_Response = SDIO_Response_No;        //该命令无响应
  SDIO_CmdInitStructure.SDIO_Wait = SDIO_Wait_No;                //关闭等待中断
  SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;            //使能命令状态机
  ...
  /*!< CMD Response: R7 */
  /* 发送 SD 卡接口条件，包含了主机支持的电压信息，并询问卡是否支持 */
  SDIO_CmdInitStructure.SDIO_Argument = SD_CHECK_PATTERN;      //检查模式
  SDIO_CmdInitStructure.SDIO_CmdIndex = SDIO_SEND_IF_COND;     //CMD8命令
  SDIO_CmdInitStructure.SDIO_Response = SDIO_Response_Short;   //该命令为短响应
  SDIO_CmdInitStructure.SDIO_Wait = SDIO_Wait_No;              //关闭等待中断
  SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;          //使能命令状态机
  SDIO_SendCommand(&SDIO_CmdInitStructure);
  ...
  /*!< If errorstatus is SD_OK it is a SD card: SD card 2.0 (voltage range mismatch)
     or SD card 1.x */
  if (errorstatus == SD_OK)  /* 如果响应了CMD55命令则是SD卡 1.x 或 v2.0 */
  {
    /*!< SD CARD */
    ...
    while ((!validvoltage) && (count < SD_MAX_VOLT_TRIAL))
    {
      ...
      SDIO_CmdInitStructure.SDIO_Argument = SD_VOLTAGE_WINDOW_SD | SDType; //sd卡电压范围和sd类型
      SDIO_CmdInitStructure.SDIO_CmdIndex = SD_CMD_SD_APP_OP_COND;         //ACMD41命令
      SDIO_CmdInitStructure.SDIO_Response = SDIO_Response_Short;
      SDIO_CmdInitStructure.SDIO_Wait = SDIO_Wait_No;
      SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;
      SDIO_SendCommand(&SDIO_CmdInitStructure);
    ...
    {
      errorstatus = SD_INVALID_VOLTRANGE;  //SDIO不支持sd卡的供电电压
      return(errorstatus);
    }
...
SD_Error SD_InitializeCards(void)
{
  ...
  /* 判断CardType是否为SDIO_SECURE_DIGITAL_IO_CARD类型的卡 */
  if (SDIO_SECURE_DIGITAL_IO_CARD != CardType)
  {
    /* 如果不是SDIO_SECURE_DIGITAL_IO_CARD类型的卡直接读取CID */
    /*!< Send CMD2 ALL_SEND_CID */
    /* 通知SD卡返回CID值 */
    SDIO_CmdInitStructure.SDIO_Argument = 0x0;
    SDIO_CmdInitStructure.SDIO_CmdIndex = SD_CMD_ALL_SEND_CID;  //CMD2
    SDIO_CmdInitStructure.SDIO_Response = SDIO_Response_Long;   //响应为长响应
      ...
      ||  (SDIO_HIGH_CAPACITY_SD_CARD == CardType))
  {
    ...
    /*!< SD Card publishes its RCA. （通知所有卡发布新 RCA）*/
    SDIO_CmdInitStructure.SDIO_Argument = 0x00;
...
SD_Error SD_SelectDeselect(uint32_t addr)
{
  ...
  SDIO_CmdInitStructure.SDIO_Argument =  addr;                  //卡地址 
  SDIO_CmdInitStructure.SDIO_CmdIndex = SD_CMD_SEL_DESEL_CARD;  //CMD7
  SDIO_CmdInitStructure.SDIO_Response = SDIO_Response_Short;    //响应为短响应
  SDIO_CmdInitStructure.SDIO_Wait = SDIO_Wait_No;               //不等待
  SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;           //命令状态机使能
  SDIO_SendCommand(&SDIO_CmdInitStructure);                     //发送命令

  errorstatus = CmdResp1Error(SD_CMD_SEL_DESEL_CARD);           //接收响应
...
SD_Error SD_ReadBlock(uint8_t *readbuff, uint32_t ReadAddr, uint16_t BlockSize)
{
  ...
  SDIO->DCTRL = 0x0; //复位SDIO数据控制寄存器

  
  if (CardType == SDIO_HIGH_CAPACITY_SD_CARD)
  {
    BlockSize = 512;
    ReadAddr /= 512;  //如果是高容量SD卡，地址是以块为单位
  }

  /* 初始化SDIO数据通道 */
  SDIO_DataInitStructure.SDIO_DataTimeOut = SD_DATATIMEOUT;
  SDIO_DataInitStructure.SDIO_DataLength = BlockSize;                 //指定要传输数据的字节数为BlockSize            
  SDIO_DataInitStructure.SDIO_DataBlockSize = (uint32_t) 9 << 4;      //对应宏SDIO_DataBlockSize_512b
...
SD_Error SD_WriteBlock(uint8_t *writebuff, uint32_t WriteAddr, uint16_t BlockSize)
{
  ...
  errorstatus = CmdResp1Error(SD_CMD_WRITE_SINGLE_BLOCK); //接收响应
...
SD_Error SD_WriteMultiBlocks(uint8_t *writebuff, uint32_t WriteAddr, uint16_t BlockSize, uint32_t NumberOfBlocks)
{
  ...
  TransferError = SD_OK;
  TransferEnd = 0;
  StopCondition = 1; //SDIO中断时用来判断是否发送停止命令
  ...
  /* 连 续 写 数 据 块 直 到STOP_TRANSMISSION 命令被发送。块长度和 WRITE_BLOCK 一致 */
  SDIO_CmdInitStructure.SDIO_Argument = (uint32_t)WriteAddr; //参数为数据地址
...
SD_Error SD_WaitWriteOperation(void)
{
  ...
  /* DMA传输结束、传输结束、传输错误三者只要有一个满足就会跳出循环 */
  while ((SD_DMAEndOfTransferStatus() == RESET) && (TransferEnd == 0) && (TransferError == SD_OK))
  {}
...
{
  if (StopCondition == 1)  //如果是多块读写（只要在多块读写函数中会被置1）
  {
    /* 发送停止命令 */
    SDIO->ARG = 0x0;
    SDIO->CMD = 0x44C;
